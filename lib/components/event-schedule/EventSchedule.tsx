...
import ExportModalImage from "../../assets/images/modal-export.png";
import ShareModalImage from "../../assets/images/modal-share.png";
import EventAdd from "../../assets/icons/event_added.svg";

const favoritedEventsThisSession = new Set();

// ICS and google cal generator
const generateCalendarExport = (events: any[], { id, description }: any) => {
  const ics = [
    `BEGIN:VCALENDAR`,
    `PRODID:${id}`,
    `METHOD:PUBLISH`,
  ...
  events.forEach((eventData: any) => {
    ...
    const { calendarTime: startOfFirstDay } = sanitizeEventTime(
      firstDay.split("-")[0]
    ) || { calendarTime: null };
    const { calendarTime: endOfLastDay } = sanitizeEventTime(
      lastDay.split("-")[1]
    ) || { calendarTime: null };
    ...
    const description = (() => {
      ...
      const allEventTimesValid = timeOfDayArray.every(
    ...
    googleCalUrl = (() => {
      ...
      if (event.Location.url)
        googleCalUrl.searchParams.append("location", `${event.Location.text}`);
    ...
    if (isMultiDayEvent) {
      ...
      const timeOfDayStart = startOfFirstDay
        ? `T${startOfFirstDay}`
        : "T000000";
      ...
      const isAllDayEvent =
        timeOfDayStart === "T000000" && timeOfDayEnd === "T000000";
      ...
      if (isAllDayEvent) {
        ...
        googleCalUrl.searchParams.append(
          "dates",
          `${start.format("YYYYMMDD")}/${endDate.format("YYYYMMDD")}`
        );
        ...
        googleCalUrl.searchParams.append(
          "dates",
          `${start.format("YYYYMMDD")}${timeOfDayStart}/${endDate.format(
            "YYYYMMDD"
          )}${timeOfDayEnd}`
        );
      }
  ...
  const calendarName =
    events.length === 1 ? events[0].event.Name : "Devconnect Week";
...
const sanitizeEventTime = (eventTime: string) => {
  ...
  if (isCorrectFormat) {
    ...
    return {
      calendarTime: `${leftPadNumber(asMoment.get("hours"))}${leftPadNumber(
        asMoment.get("minutes")
      )}${leftPadNumber(asMoment.get("seconds"))}`,
      normalizedEventTime,
    };
  }
...
const getFormattedEventData = (event: any, day?: any) => {
  ...
  const timeOfDayArray =
    event["Time of Day"] && event["Time of Day"].split(",");
  ...
  const timeOfDay =
    timeOfDayArray &&
    timeOfDayArray[shouldRepeatTimeOfDay ? 0 : timeOfDayIndex];
  ...
  const scheduleHelpers = React.useMemo(() => {
    ...
    sortedEvents
      ...
      .forEach((event: any) => {
        ...
        const firstDay = eventBoundaries.min
          ? eventBoundaries.min.diff(min, "days")
          : 0;
        const lastDay = eventBoundaries.max
          ? eventBoundaries.max.diff(min, "days") + 1
          : 1;
...
const CalendarModal = ({
  ...
  return (
    <Modal
      className={`${css["add-to-calendar-modal"]} ${css["global-styles"]}`}
      ...
      <ModalContent>
        <div className={`${css["add-to-calendar-modal-content"]}`}>
          ...
          <div className={css["description"]}>
            <p className="bold margin-bottom-much-less">
              Export Schedule to Calendar (.ICS)
            </p>
            <p className="small-text">
              Download the .ICS file to upload to your favorite calendar app.
            </p>
          </div>
          ...
          <div className={css["buttons"]}>
            {(() => {
              if (favorites && favorites.favoriteEvents.length > 0) {
                ...
                return (
                  <a {...icsAttributes} className="orange-fill sm small-text">
                    <Button color="purple-1" fill>
            ...
            <a {...icsAttributes} className="orange sm small-text">
              <Button color="purple-1" fill>
                <EventAdd /> {hasMultipleEvents ? "All events" : "Download"}{" "}
                (.ICS)
              </Button>
            </a>
          ...
          <p className={css["notice"]}>
            ...
            <span className="tiny-text orange uppercase bold">
              Event information will not be updated once you export the schedule
              to your personal calendar. PLEASE VISIT THE SCHEDULE CLOSER TO
              EVENT DATE TO MAKE SURE YOUR EVENTS HAVE NOT HAD ANY CHANGES
            </span>
          </p>
        </div>
      </ModalContent>
...
const Favorite = ({ event, favorites, noContainer, sharingDisabled }: any) => {
  const [hovered, setHovered] = React.useState(false);
  const isFavorited = favorites.favoriteEvents.some(
    (favoritedEvent: any) => event.ShortID === favoritedEvent
  );

  if (sharingDisabled) return null;
    ...
    <div
      ...
      onClick={(e) => {
        if (favorites.sharedEvents) {
          alert(
            "You are currently viewing a shared schedule. Return to your schedule to edit your favorites."
          );
        }

        if (isFavorited) {
          favorites.setFavoriteEvents(
            favorites.favoriteEvents.filter(
              (favoriteEvent: any) => favoriteEvent !== event.ShortID
            )
          );
        } else {
          favorites.setFavoriteEvents(
            favorites.favoriteEvents.concat(event.ShortID)
          );
  ...
  return (
    <div
      className={`${css["hover-overlay"]} ${
        isFavorited ? css["favorited"] : ""
      }`}
    >
...
const useFavorites = (events: any, edition: Edition): any => {
  ...
  const exportFavorites = () => {
    ...
    url += `?edition=${encodeURIComponent(edition)}&share=${encodeURIComponent(
      shareParams
    )}`;

    if (shareTitleInput)
      url += `&share_title=${encodeURIComponent(shareTitleInput)}`;

    navigator.clipboard.writeText(url);
  };

  const exitSharedMode = () => {
    router.push(window.location.origin + window.location.pathname, undefined, {
      shallow: true,
    });
...
const Timeline = (props: any) => {
  ...
  React.useEffect(() => {
    ...
    if (anchor) {
      ...
      const selectedEvent = sortedEvents.find(
        (event: any) => event.ShortID === decoded
      );
  ...
  const events = sortedEvents.map((event: any, index: number) => {
    ...
    const offsetFromFirstEventInSchedule = startDay.diff(
    ...
    if (isBlockingEvent) {
      ...
      return (
        <React.Fragment key={event.Name + offsetFromFirstDay}>
          <div style={gridPlacement} {...draggableAttributes}>
            {props.renderBlockingEvent()}
          </div>
        </React.Fragment>
      );
    }
      ...
      <React.Fragment key={event.Name + offsetFromFirstDay}>
        <div
          className={(() => {
            ...
            if (props.favorites.sharedEvents && !isFavorited)
              className += ` ${css["faded"]}`;

            if (props.edition === "istanbul") {
              className += ` ${css["domain-based"]}`;
              if (props.event["Domain"]) className += ` ${css["domain"]}`;
            } else if (props.edition === "amsterdam") {
              className += ` ${css["difficulty-based"]} ${
                css[props.event["Difficulty"]]
              }`;
            } else {
          ...
          <div className={css["content"]}>
            {/* {event["Stable ID"] === "Cowork" && (
              <div className={css["image"]}>
                {(() => {
                  if (props.edition === "istanbul")
                    return <DevconnectIstanbul style={{ width: "30px" }} />;
                  if (props.edition === "amsterdam")
                    return <DevconnectAmsterdam style={{ width: "50px" }} />;
                })()}
              </div>
            )} */}
            <div className={css["content-inner"]}>
              <div className={css["top"]}>
                <div className={css["title-bar"]}>
                  <p
                    className={`large-text-em bold ${css["title"]} ${
                  ...
                  <Favorite
                    ...
                    sharingDisabled={props.sharingDisabled}
                  />
                </div>
                ...
                {event["Time of Day"] && (
                  <div className={css["when"]}>
                    {Array.from(Array(totalDays)).map((_, index: number) => {
                      ...
                      const useDayIndicator =
                        !!timeOfDayArray[1] && totalDays > 1;
                      ...
                      if (!time) return null;
                      if (shouldRepeatTimeOfDay && isMultiDayEvent && index > 0)
                        return null;
                          ...
                          <p className="bold">
                            ...
                            {useDayIndicator && (
                              <>
                                <br />
                                <span
                                  className={`${css["which-day"]} small-text-em`}
                                >
                                  Day {index + 1}
                                </span>
                              </>
  ...
  return (
    <>
      ...
      {(() => {
        const selectedEvent = sortedEvents.find(
          (event: any) => event.ID === eventModalOpen
        );
      ...
      <SwipeToScroll noBounds>
        <div className={css["timeline"]}>
          ...
          {Array.from(Array(scheduleDuration)).map((_, index: number) => {
            ...
            const day = moment
              .utc(defaultSortEvents[0].Date.startDate)
              .add(index, "days");
            ...
            return (
              <div
                className={className}
                key={index}
                ref={dayIsActive ? todayRef : undefined}
              >
  ...
  return (
    <div className={css["meta"]}>
      {props.event["General Size"] &&
        props.event["General Size"].length > 0 && (
          <div
            className={`small-text-em`}
      ...
      {props.event.Difficulty && (
        <div
          className={`small-text-em ${css["difficulty"]} ${
            css[props.event.Difficulty]
          }`}
        >
          {props.event.Difficulty}
        ...
        {props.event.Category &&
          ...
          props.event.Category.map((category: any) => {
            return (
              <div
                key={category}
                className={`tag tiny-text-em ${
                  category === "Virtual Event" ? css["is-virtual"] : ""
                }`}
              >
                {category}{" "}
                {category === "Virtual Event" && (
                  <span className={css["virtual"]}>🌐</span>
                )}
              </div>
            );
          })}
    ...
    <div className={`${css["event-links"]} tiny-text uppercase`}>
      ...
      {event["Stream URL"] && (
        <Link
          ...
          indicateExternal
          className="button xs orange-fill"
        >
          Stream
        </Link>
      )}
...
const LearnMoreModal = (props: {
  ...
  return (
    <Modal
      open={props.open}
      close={props.close}
      className={`${css["learn-more-modal"]} ${
        css[`edition-${props.edition}`]
      } bg-white ${css["global-styles"]}`}
      noCloseIcon
    >
      <ModalContent>
  ...
  return (
    <>
      ...
      >
        <p>Learn More →</p>
        {props.event["Attend"] && (
          <p className={css["attend-details"]}>{props.event["Attend"]}</p>
        )}
      </div>
    </>
  );
...
const ListTableHeader = () => {
  return (
    <div
      className={`uppercase bold ${css["list-table-header"]} ${css["list-grid"]}`}
    >
    ...
    <div>
      <div className={className} onClick={() => setOpen(!open)}>
        <div className={css["date"]}>
          <p className="thin big-text uppercase bold">
            {dayIsActive ? "TODAY" : day}
          </p>
          <p className="thin small-text uppercase bold">{date}</p>
        </div>

        <div className={css["toggle-open"]}>
...
const ListEventDesktop = (props: any) => {
  ...
  return (
    <div
      className={(() => {
        let className = `${css["event-in-table"]} ${
          css[props.event["Stable ID"]]
        }`;
      ...
      <div className={`${css["list-grid"]} ${css["content"]} `}>
        <div className={`${css["date"]} ${css["col-1"]}`}>
          <div>
            <p className="bold">
              {formattedDate} — <br />{" "}
              <span className="small-text">{timeOfDay}</span>
          ...
          {isMultiDayEvent && (
            <div
              className={`tag bold purple tiny-text-em ${css["multi-day-indicator"]}`}
            >
              Multi-day Event
            </div>
          )}
          ...
          <div>
            {props.event.URL ? (
              <Link
                ...
                indicateExternal
                className={`${css["title"]} big-text bold uppercase`}
              >
                {props.event.Name}
              </Link>
            ) : (
              <p className={`${css["title"]} big-text bold uppercase`}>
                {props.event.Name}
              </p>
            )}
            ...
            {props.event["Brief Description"] && (
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: htmlDecode(
                    htmlEscape(props.event["Brief Description"])
                  ),
                }}
              />
            )}
        ...
        <div className={`${css["organizers"]} ${css["col-3"]}`}>
          {props.event["Organizer"] && (
            <p className={`${css["organizers"]}`}>{props.event["Organizer"]}</p>
          )}
        </div>
          ...
          {props.event["Attend"] &&
            (props.event["URL"] ? (
              <Link
                href={props.event.URL}
                indicateExternal
                className={`${css["ticket-availability"]} small-text`}
              >
                {props.event["Attend"]}
              </Link>
            ) : (
              <p className={`${css["ticket-availability"]} small-text`}>
                {props.event["Attend"]}
              </p>
            ))}
        </div>
  ...
  return (
    ...
    >
      <div className={css["content"]}>
        <div className={css["split"]}>
          {props.event.URL ? (
            <Link
              href={props.event.URL}
              indicateExternal
              className={`${css["title"]} large-text uppercase bold`}
            >
              {props.event.Name}
            </Link>
          ) : (
            <p className={`${css["title"]} large-text uppercase bold`}>
              {props.event.Name}
            </p>
          )}
          ...
          <div className={css["share-icon"]}>
            <CopyToClipboard
              url={`${window.location.href.split("#")[0]}#${encodeURI(
                props.event.ShortID
              )}`}
            />
          </div>
        ...
        {isMultiDayEvent && (
          <div className={`tag purple tiny-text ${css["multi-day-indicator"]}`}>
            Multi-day Event
          </div>
        )}
        {/* 
        {(() => {
          ...
          if (props.edition === "istanbul")
            return (
              <DevconnectIstanbul style={{ width: "50px", display: "block" }} />
            );
          if (props.edition === "amsterdam")
            return (
              <DevconnectAmsterdam
        ...
        {props.event["Brief Description"] && (
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: htmlDecode(htmlEscape(props.event["Brief Description"])),
            }}
          />
        )}
        ...
        {props.event["Stable ID"] === "Easter" && props.timeline && (
          <img
            src="https://c.tenor.com/thDFJno0zuAAAAAd/happy-easter-easter-bunny.gif"
            alt="Easter egg"
            width="100%"
          />
        )}
        ...
        <div className={css["split"]}>
          {props.event["Organizer"] && (
            <p className={`uppercase ${css["organizers"]}`}>
              {props.event["Organizer"]}
            </p>
          )}
        </div>
...
const useFilter = (events: any, edition: Edition, favorites: any) => {
  ...
  const [showOnlyDomainSpecific, setShowOnlyDomainSpecific] =
    ...
    <div className={`${css["filter"]} small-text`}>
                  ...
                  <div className={css["list-toggle-item"]}>
                    ...
                    <label className={css["toggle"]}>
                      <Toggle
                        ...
                        icons={false}
                        onChange={() =>
                          props.setShowOnlyDomainSpecific(
                            !props.showOnlyDomainSpecific
                          )
                        }
                      />
                    </label>
                  ...
                  <div className={css["list-toggle-item"]}>
                    ...
                    <label className={css["toggle"]}>
                      <Toggle
                        ...
                        onChange={() =>
                          props.setHideSoldOut(!props.hideSoldOut)
                        }
                      />
                    </label>
                  </div>
      ...
      <div className={css["experience"]}>
        ...
        <div className={css["tags"]}>
          {Array.from(props.filterableValues["Difficulty"]).map(
            (filterableValue: any) => {
              const isActive = props.difficultyFilter.includes(filterableValue);

              const onChange = () => {
                if (isActive) {
                  props.setDifficultyFilter(
                    props.difficultyFilter.filter(
                      (filter: any) => filter !== filterableValue
                    )
                  );
                } else {
                  props.setDifficultyFilter(
                    props.difficultyFilter.concat(filterableValue)
                  );
                }
              };
              return (
                <div
                  key={filterableValue}
                  className={`tag xs tiny-text clickable ${
                    isActive ? "slick-purple" : ""
                  }`}
                  onClick={onChange}
                >
                  {filterableValue}
      ...
      <div className={css["secondary-filter"]}>
        <ListComponent
          className="border-top"
          items={Array.from(props.filterableValues["Category"]).map(
            (filterValue: any) => {
                          ...
                          if (isChecked) {
                            props.setCategoryFilter(
                              props.categoryFilter.filter(
                                (filter: any) => filter !== filterValue
                              )
                            );
                          } else {
                            props.setCategoryFilter(
                              props.categoryFilter.concat(filterValue)
                            );
                          }
                        }}
                      />
      ...
      <div className={css["secondary-filter"]}>
        <ListComponent
          className="border-top"
          items={Array.from(props.filterableValues["Attend"]).map(
            (filterValue: any) => {
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (isChecked) {
                            ...
                            props.setStatusFilter(
                              props.statusFilter.concat(filterValue)
                            );
                          }
                        }}
                      />
  ...
  return (
    <div className={css["expand-container"]}>
      <button
        ...
        onClick={() =>
          Object.values(props.accordionRefs.current).forEach(
            (acc: any) => acc && acc.open && acc.open()
          )
        }
      >
        <span>
      ...
      <button
        ...
        onClick={() =>
          Object.values(props.accordionRefs.current).forEach(
            (acc: any) => acc && acc.close && acc.close()
          )
        }
      >
        <span>
    ...
    <SwipeToScroll noBounds scrollIndicatorDirections={{ right: true }}>
      <div className={css["actions"]}>
          ...
          onClick={() => {
            ...
            if (yes) {
              ...
              const merged = Array.from(
                new Set(currentFavorites.concat(sharedFavorites))
              );
        ...
        <button
          ...
          onClick={() =>
            props.favorites.setOnlyShowSharedEvents(
              !props.favorites.onlyShowSharedEvents
            )
          }
          {...draggableLinkAttributes}
        >
...
const Schedule: NextPage = scheduleViewHOC((props: any) => {
  ...
  if (favorites.sharedEvents && favorites.onlyShowSharedEvents) {
    events = events.filter((event: any) =>
      favorites.sharedEvents.includes(event.ShortID)
    );
  }
  ...
  React.useEffect(() => {
    Object.values(accordionRefs.current).forEach(
      (acc) => acc && acc.open && acc.open()
    );
  }, [
      ...
      >
        <div className={css["hero-content"]}>
          <p className="uppercase extra-large-text bold secondary title">
            {(() => {
              if (props.edition === "istanbul")
                return "Schedule - Istanbul 2023";
              if (props.edition === "amsterdam")
                return "Schedule - Amsterdam 2022";
            })()}
          </p>
          <Link
        ...
        <div className="section">
          <Alert title="Important" className={`sm ${css["alert"]}`}>
            <p className="bold small-text padding-top-less padding-bottom-less">
              👉 Remember,{" "}
              <u>each event during Devconnect is independently hosted</u> and
              you will require tickets for each event you wish to attend. You
              will find each ticketing information below.
            </p>
          </Alert>
        </div>
      )} */}
        ...
        <div className="section">
              ...
              <div className={css["top-bar"]}>
                <div className={css["second-row"]}>
                  <div className={`${css["view"]} small-text`}>
                    ...
                    <Button
                      color={
                        scheduleView === "timeline" ? "orange-1" : undefined
                      }
                      className="xs"
                      onClick={() => setScheduleView("timeline")}
                    >
                      <p className={`${css["text"]} small-text mr-2`}>
                        Timeline
                      </p>
                      <CalendarIcon />
                    </Button>
                    ...
                    <Button
                      color={
                        scheduleView === "timeline" ? undefined : "orange-1"
                      }
                      className="xs ml-4"
                      onClick={() => setScheduleView("list")}
                    >
                      <p className={`${css["text"]} small-text mr-2`}>List</p>
                      <ListIcon style={{ fontSize: "0.8em" }} />
                    </Button>
                    {/* <button
                        className={`${
                          scheduleView === "timeline" && css["selected"]
                        } white button xs`}
                        onClick={() => setScheduleView("timeline")}
                      >
                        <CalendarIcon />
                      ...
                      <button
                        className={`${
                          scheduleView === "list" && css["selected"]
                        } white button xs`}
                        onClick={() => setScheduleView("list")}
                      >
                        <ListIcon style={{ fontSize: "0.8em" }} />
                  ...
                  {props.edition === "istanbul" && (
                    <div className={css["types"]}>
                      ...
                      <div className={css["all-welcome"]}>
                        <p>
                          <span className={css["indicator"]}>⬤</span>Ecosystem
                          Events
                        </p>
                      </div>
                      <div className={css["virtual"]}>
                        <p>
                          <span className={css["indicator"]}>⬤</span>Virtual
                          Events
                        </p>
                      </div>
                      <div className={css["intermediate"]}>
                        <p>
                          <span className={css["indicator"]}>⬤</span>Other
                          Events
                        </p>
                      </div>
                    </div>
                  ...
                  {favorites.sharedEvents === null && (
                    <div>
                      <Button
                        color={props.buttonColor || "purple-1"}
                        className="mx-4"
                        fill
                        onClick={() => setCalendarModalOpen(true)}
                      >
                        <div className="mr-2">Export (.ics)</div>
                        <ScheduleDownloadIcon className="text-lg icon" />
                      </Button>

                      {props.sharingDisabled || (
                        <Button
                          color={props.buttonColor || "purple-1"}
                          fill
                          onClick={() => setOpenShareModal(true)}
                        >
                          <div className="mr-2">Share Schedule Snapshot</div>{" "}
                          <TwirlIcon className="text-lg icon" />
                        </Button>
                      )}
            ...
            >
              <ModalContent>
                <div className={css["share-schedule-modal"]} draggable="false">
                    ...
                    <div className={css["content"]}>
                      ...
                      <div className={css["standard-input"]}>
                        <input
                          ...
                          onChange={(e) =>
                            favorites.setShareTitleInput(e.target.value)
                          }
                          placeholder="Name"
                        />
                        <SearchIcon />
                  ...
                  <div className={css["description"]}>
                    <p className="bold margin-bottom-much-less">
                      Share a snapshot of your personal schedule
                    </p>
                    <p className="small-text">
                      Create a shareable link of your favorited events on this
                      device to share with others, or to view/import on another
                      device.
                    </p>
                  </div>
                  ...
                  <div className={css["action"]}>
                    ...
                    <p className={`${css["preview"]}`}>
                      ...
                      <span className="orange bold underline">
                        {favorites.shareTitleInput || "a shared schedule"}
                      </span>
                      &nbsp;snapshot.
                    </p>
                  ...
                  <p className={css["notice"]}>
                    ...
                    <br />
                    <span className="tiny-text orange uppercase bold">
                      This will be a snapshot of your currently favorited events
                      on this device. Any subsequent updates to your favorites
                      won&apos;t change the snapshot shared.
                    </span>
                  </p>
                </div>
              </ModalContent>
          ...
          <div className={css["first-row-above-schedule"]}>
            <div className={css["filter-toggle"]}>
              <button
                className={filterOpen ? css["active"] : ""}
            ...
            {scheduleView === "timeline" && (
              <p className={`small-text bold uppercase ${css["swipe"]}`}>
                Hold and drag schedule for more →
              </p>
            )}
          ...
          <div
            className={`${css["schedule-wrapper"]} ${
              scheduleView === "timeline" ? css["timeline-wrapper"] : ""
            }`}
          >
            {filterOpen && (
              <>
            ...
            {events.length === 0 ? (
              <div className={css["no-results"]}>
                <Image
                  src={FilterMiss}
                  alt="Guy unable to find an Ethereum artifact"
                />
                  ...
                  <div className={css["currently-viewing"]}>
                    ...
                    <p className={css["snapshot-title"]}>
                      <span className="orange">
                        {favorites.sharedTitle || "a shared schedule"}
                      </span>{" "}
                      snapshot
                    </p>
                  </div>
