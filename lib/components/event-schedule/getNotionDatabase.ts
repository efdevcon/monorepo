...
const formatResult = (language: "en" | "es") => (result: any) => {
  ...
  // TODO: no longer needed? or maybe in the future?
  allKeys.forEach((key, index) => {
    ...
    if (isSpanish && spanishValue !== undefined) {
      parsedValue = notionDatabasePropertyResolver(
        spanishValue,
        spanishKeyCandidate
      );
    } else {
...
const getNotionDatabase = async (
  ...
  const isColombiaBlockhainWeek =
    databaseID === "cc11ba1c0daa40359710c0958da7739c";
