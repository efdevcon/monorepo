...
export const Tabs = React.forwardRef((props: TabsProps, ref: any) => {
  ...
  const tabFromQueryString = searchParams.get("tab");

  const defaultTab = props.children
    ? findFirstValidTab(props.children)?.props?.title
    : "";
  ...
  React.useLayoutEffect(() => {
    if (
      tabFromQueryString &&
      props.children &&
      isValidTab(props.children, tabFromQueryString)
    ) {
  ...
  useQueryStringer(
    props.useQuerystring ? { tab: activeTab } : {},
    props.useQuerystring,
    true
  );

  let tabContentClassName = css["tab-content"];
  if (props.tabContentClassName)
          ...
          props.children.map((child: any) => {
            ...
            return (
              <li
                ...
                onClick={() => onSelectTab(childProps.title)}
              >
                {childProps.title}
              </li>
            );
