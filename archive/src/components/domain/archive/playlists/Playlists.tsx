...
export const Playlists = () => {
  ...
  const sliderPropsMostPopular = useSlider(
    getSliderSettings(mostPopular.videoCount)
  );
      ...
      <div className="content">
        <div className="border-top margin-top padding-bottom">
          <Slider
            ...
            sliderProps={sliderPropsOpening}
            title="Devcon 6 Opening"
          >
            {limit15(
              opening.videos,
              opening,
              sliderPropsOpening[1].canSlide
        ...
        <div className={css["playlists"]}>
          <div className="border-top padding-bottom">
            <Slider
              className={css["slider"]}
              sliderProps={sliderDevcon6}
              title="Devcon 6"
            >
              {limit15(devcon6.videos, devcon6, sliderDevcon6[1].canSlide).map(
                (item: any, i: number) => {
                  if (item.playlist) return item.render();
          ...
          <div className="border-top padding-bottom">
            <Slider
              className={css["slider"]}
              sliderProps={sliderPropsLatest}
              title="Devcon 5"
            >
              {limit15(
                latest.videos,
                latest,
                sliderPropsLatest[1].canSlide
              ).map((item: any, i: number) => {
                if (item.playlist) return item.render();
          ...
          <div className="border-top padding-bottom">
            <Slider
              className={css["slider"]}
              sliderProps={sliderPropsMostPopular}
              title="Most Popular"
            >
              {limit15(
                mostPopular.videos,
                mostPopular,
                sliderPropsMostPopular[1].canSlide
              ).map((item: any, i: number) => {
                if (item.playlist) return item.render();
