...
export default function UpdatePage({ params }: { params?: { name: string; id: string } }) {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [pageId, params, router.query, router.isReady]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          {fields.filter(field => field.mode === 'edit').some(field => field.value && field.value.trim() !== '') ? 'Update your information' : 'Submit your information'}
        </p>
      </div>
      ...
      {fields.map((field) => (
                ...
                >
                  {fileUploads[field.name]?.preview ? (
                    ...
                    <div style={{ width: '100%' }}>
                      ...
                      <p style={{ margin: '0', fontSize: '0.9rem', color: '#666' }}>
                        {fileUploads[field.name]?.preview ? getFileNameFromUrl(fileUploads[field.name].preview!) : 'File uploaded'}
                      </p>
        ...
        <div style={{ textAlign: 'center', marginTop: '2rem' }}>
          ...
          >
            {fields.filter(field => field.mode === 'edit').some(field => field.value && field.value.trim() !== '') ? 'Update' : 'Submit'}
      </button>
        </div>
    </form>
