...
/** Resolve {baseId, tableId} from a view's first column — no forms-meta needed. */
async function resolveTableViaColumns(viewId: string): Promise<{ baseId: string; tableId: string }> {
...
/** Resolve a view id to its {baseId, tableId} for READING records. */
export async function resolveViewTable(viewId: string): Promise<{ baseId: string; tableId: string }> {
  try {
    ...
    return resolveTableViaColumns(viewId)
  }
}
