...
import type { ConferenceEvent, Session, Speaker } from "@/data/models";
 ...
 * cached sessions list — offline-safe, since both lists live in the EventStore.
 */
...
interface Derived {
  ...
  byId: Map<string, DecoratedSpeaker>;
  topicOptions: ReturnType<typeof deriveTopicOptions>;
  typeOptions: string[];
}
...
// One derivation per store snapshot, shared by every mount. The inputs are the
// snapshot's arrays, whose identity only changes on a sync, so a module-level
// memo (rather than a per-component useMemo) means the join is computed once
// per sync instead of once per page mount.
let memoInputs: {
  speakers: Speaker[];
  sessions: Session[];
...
function derive(
  ...
  if (
    memoDerived &&
    memoInputs &&
    memoInputs.speakers === speakers &&
    memoInputs.sessions === sessions &&
    memoInputs.event === event
  ) {
    return memoDerived;
  }
  ...
  const decorated: DecoratedSpeaker[] = speakers
    .map((speaker) => {
      ...
      const tags = [
        ...new Set(own.flatMap((s) => s.tags ?? []).map((t) => t.trim())),
      ]
        .filter(Boolean)
  ...
  memoInputs = { speakers, sessions, event };
  memoDerived = { decorated, byId, topicOptions, typeOptions };
  return memoDerived;
}
...
export function useSpeakersData() {
  ...
  // Identity-stable per snapshot (see `derive`): list consumers feed these
  // straight into their own memos, so identity churn here would re-filter the
  // whole page each render (project Set/Map-identity gotcha).
  const derived = useMemo(
    () => derive(speakers, sessions, event),
