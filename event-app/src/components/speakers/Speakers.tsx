...
export function Speakers() {
  ...
  const { ids: interestedIds } = useInterestedSpeakers();
  const {
    search,
    setSearch,
    topics,
  ...
  useEffect(() => {
    ...
    const measure = () => {
      raf = 0;
      const headerOffset = isDesktopNow() ? 65 : 56;

      const rows = stickyRowsRef.current;
      setRowsStuck(
        !!rows && rows.getBoundingClientRect().top <= headerOffset + 1
      );
      ...
      if (rail) {
        ...
        // and the 8px inset keeps Z clear of the viewport edge. Space is
        // measured from the rail's live top, so the compact stack also caps
        // to what actually fits below its resting position — on viewports
        // too short for it, the cells flex-shrink evenly instead of letting
        // the tail letters clip.
        const viewportH = window.visualViewport?.height ?? window.innerHeight;
        // Mobile reserves room for the floating bottom nav pill (24px offset
        // + ~52px pill + safe-area headroom) — the rail is unusable if the
        // tail letters slide behind it. Desktop just keeps an 8px edge inset.
        ...
        // Scroll-spy: the letter of the topmost section in view. Sections
        ...
        const resolved = current ?? letterGroups[0]?.letter ?? null;
      ...
      if (aside && sidePanelOpen) {
        ...
        const gapTarget = viewportH - top - 32;
    ...
    >
      {speakers.map((d) => (
        <SpeakerCard
          ...
          onOpen={(id) =>
            selectSpeaker(id === selectedSpeakerId ? null : id)
          }
        />
      ))}
    </div>
