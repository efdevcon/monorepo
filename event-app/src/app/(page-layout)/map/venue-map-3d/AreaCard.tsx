...
import { categoryIdFor } from "./pois";
...
/**
 * The bottom card that opens when an area is tapped: theme icon (stages: in
 * the disc on the top edge; everything else: inline left of the text — Scott,
 * 2026-09-21), name, the floor it is on and a short description, plus — for footprints backed by
 ...
 * follows the camera without React in the loop. For a group pick (`group`)
 * the desktop card sits bottom-centre instead, clear of the footprints.
 */
export function AreaCard({
  ...
  area: Area | null;
  levels: PlanLevel[];
  /** The area is one of several highlighted together (Find/Search/legend group): desktop parks the card bottom-centre so none of them is covered. */
  group?: boolean;
  onClose: () => void;
  ...
  const stage = shown !== null && categoryIdFor(shown.id.slice(shown.id.indexOf("/") + 1)) === "stages";
  const discIcon = stage ? icon : null;
  const inlineIcon = stage ? null : icon;
    ...
    >
      {discIcon && (
        ...
        <div className="pointer-events-none absolute left-1/2 top-0 flex size-16 -translate-x-1/2 -translate-y-[70%] items-center justify-center rounded-full bg-white/95 backdrop-blur">
          ...
          <img src={iconUrl(discIcon)} alt="" className="size-12 object-contain" />
        </div>
      )}
      ...
      <div className="flex items-start gap-3 pr-3">
        {inlineIcon && (
          // eslint-disable-next-line @next/next/no-img-element -- static PNG under public/, no optimisation wanted
          <img src={iconUrl(inlineIcon)} alt="" className="size-12 shrink-0 object-contain" />
        )}
