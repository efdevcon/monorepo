 ...
 * was 40 + a 44px pseudo hit box), icon + 16px label (2026-09-21, was 14), lavender + purple border
...
export function ControlPill({ icon: Icon, label, open, onClick, trigger }: { icon: LucideIcon; label: string; open: boolean; onClick: () => void; trigger: string }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "pointer-events-auto relative z-10 flex h-11 cursor-pointer items-center gap-1.5 rounded-full border pl-3 pr-4 text-[16px] font-bold leading-none text-dc-purple shadow-[0_1px_3px_rgba(22,11,43,0.12)] backdrop-blur transition-colors duration-150 ease-out",
