...
- **Sign-in state must reach the server.** The browser session lives in localStorage (auth-js), but the install bridge is decided server-side from the auth cookie. `useUser` mirrors every sign-in and token refresh into that cookie through `POST /api/auth/session` and clears it on sign-out (`src/data/auth/sessionCookie.ts`); after an OTP sign-in in an iOS browser tab it reloads the page so Safari's "Add to Home Screen" sees the personalised manifest. The desktop install card's QR (`InstallHeroCard`) embeds the same bridge link when signed in, minted with a 10-minute `ttlMs` via `POST /api/manifest-bridge` and re-minted every 8 minutes, so a scan lands signed in; the token is reusable until it expires, hence the short life. Don't add sign-in paths that skip this. iOS Safari cannot install programmatically (no `beforeinstallprompt`, the Web Share sheet has no Add to Home Screen action, WebKit has no Web Install API): the Install button shows guided steps, per iOS major version because Share moves with every release (`safariShareStep` in `InstallAppButton.tsx`, version from `iosMajorVersion()`, which trusts WebKit feature checks over the User-Agent because Safari 26+ freezes the UA's OS token); don't re-try a share-sheet shortcut.
- **Install nudge** (`InstallHeroCard`, `InstallCompactCard`, `InstallAppButton`): shown in every browser that is not the installed app, desktop included since 2026-09-24. Home shows the hero (dismissible); My Devcon shows it only once signed in, the hero with the QR on desktop and the compact one-row card on phones (`useIsDesktop` picks), never on the sign-in page (2026-09-25). Desktop gets no Install button anywhere (the hero's, Home's and My Devcon's bottom buttons and the sign-in one are `lg:hidden`); its story is the hero's QR code, which encodes this deployment's origin and, when signed in, the sign-in bridge (10-minute token, re-minted every 8 minutes) so the phone lands signed in. The how-to modal has desktop steps (Chrome/Edge address-bar icon or menu, Safari Add to Dock, none for Firefox). `NotificationsHeroCard` never shows while the install card does (`useInstallHeroVisible`).
- **Sign-in email field** is `type="text"` + `inputMode="email"` + `autoComplete="email"` inside a real `<form>`, on purpose: WebKit (Safari and Chrome on iOS) turns autocorrect off for `type="email"`, and iOS text-replacement shortcuts ride on autocorrect, so they never expanded there; browsers and password managers also only offer saved emails to a named, autocomplete-typed field in a form. Validation is `EMAIL_FORMAT`, not the input type. Don't change it back.
- **Current time**: never call `Date.now()` / `new Date()` directly in components. Use the shared `useNow`/`useNowMs` hooks (`src/hooks/useNow.ts`) so time can be mocked with `?mockNow=` / `?mockSpeed=` query params. For content dated against the real world rather than event time (announcements), use `useRealWorldNowMs` — it opts out of the per-deployment event-start auto-mock, which would otherwise let the selected dataset (e.g. devcon-7 → Nov 2024) decide whether today's announcements are visible. Announcements follow the same clock (`useAnnouncements`, `AnnouncementCard` use `useNowMs`, including the preview's event-start mock) since 2026-09-24, so the inbox, Home and the schedule agree under `?mockNow=`; the Notion test announcements are dated inside the devcon-7 mock window (Nov 2024) for that and must be removed or re-dated before launch, because the feed is shared by every dataset and reads as old news on the real clock.
...
- **No icon beside a heading, title, eyebrow or caption.** Text labels stand alone in this design language; a decorative purple glyph next to a title is the one pattern reviewers strip out every round. Icons belong in buttons and pills (16px, purple, beside the label), in meta rows (clock, pin, calendar), and in status notes.
...
Announcements with the Notion `Push` checkbox go out as web push at their Send At time; the inbox stays the source of truth (push is best-effort). Pipeline: `src/app/api/push/service.ts` (claim/fan-out/prune design notes in its header) + routes under `src/app/api/push/`; SW handlers at the bottom of `src/sw.ts` (Declarative Web Push JSON for Safari 18.4+, classic handler elsewhere). Opt-in UI: the Notifications page (`/notifications`; `/announcements` redirects, the API keeps `/api/announcements`) has a "Settings" entry (header pill on mobile, `NotificationSettingsLink` beside the h1 on desktop) opening `NotificationSettingsModal` with two peer iOS-style `Switch`es, **Announcements** (on by default once permission is granted) and **Session reminders** (opt-in) — no master switch; the first switch turned on asks for permission and subscribes with only that type, turning the last one off unsubscribes (`push.setPref`). Both flags are per device: columns `announcements` / `reminders` on `devcon8_push_subscriptions` (migration `20260923120000`), written by `POST`/`PATCH /api/push/subscriptions`, read by `POST /api/push/subscriptions/prefs`, carried over by the rotate route; the broadcast filters `announcements = true`, the reminders claim RPC filters `reminders`. One `usePushSubscription()` instance per app, provided by `src/data/push/PushProvider.tsx` (`usePush()`) — never auto-prompt: `Notification.requestPermission` is the first await of `subscribe()` and must run inside the tap. Home's install slot has a second step, `NotificationsHeroCard` (`src/components/home/`): installed or push-capable, signed in and push still off, it asks to turn notifications on (announcements only; dismissal remembered in the Dexie prefs table) and disappears once push is on or denied. The first launch of the installed app shows `PushOnboardingSheet` (`src/components/onboarding/`) once per device after the intro splash: an explainer with "Turn on notifications" (announcements on, reminders left to settings) or "Not now"; browser tabs never see it (preview with `?previewPushSheet=1` outside production). The dispatcher is `netlify/functions/push-dispatch.mts` (every minute → secret-gated `/api/push/dispatch`; idempotent, crash-reclaim after 10 min for announcements and 2 min for reminders — `STALL_MS` / `REMINDER_STALL_MS` in `service.ts`). Team test-sends: `POST /api/push/test {id}` (@ethereum.org only, doesn't consume the row's status). Env: `NEXT_PUBLIC_VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` (one keypair forever — rotating orphans every subscription), `PUSH_DISPATCH_SECRET`. Subscriptions live in `devcon8_push_subscriptions`. Note: the SW is off under `pnpm dev`, so subscribe/receive needs `pnpm preview` (build + serve) or a deploy.

**Session reminders** ride the same dispatcher: 10 minutes before a starred session starts (`REMINDER_LEAD_MS` in `src/data/reminders/reminders.ts`, the pure module shared by server, client and `data:test`; every number in copy derives from it), the account's reminder-enabled devices get "<title> starts in 10 minutes at 13:00, on <stage>" (tap → `/schedule/<id>`). Capacity per start time is lead × `MAX_REMINDERS_PER_RUN` (1,500 user-session pairs), and the reminder stall window must stay well under the lead or a crashed run is never retried. Server: `src/app/api/push/reminders.ts` joins synced stars (`devcon8_interests`) to subscriptions by `user_id`; the live bundle decides what is due each minute; the claim is the RPC `devcon8_session_reminders_claim` (the row in `devcon8_session_reminders` IS the claim, keyed `(user, event, session)`, so a slot that moves after the send is not re-sent). Events: `PUSH_REMINDER_EVENTS` (dataset keys) or the deployment default. Push is best-effort only: the Notifications inbox derives the same items on-device from local stars + catalogue + `useNowMs` (`useSessionReminders`) and shows them in ONE timeline with the announcements (no tabs; merged by `src/data/announcements/inboxItems.ts`, grouped by venue day, reminder cards lavender with a text-only "Session reminder" eyebrow); the home preview (`AnnouncementsSection`) shows up to three items, newest first, always including the latest announcement and counting a reminder only while its session has not ended, so it works offline, signed out, under `?mockNow=` and whatever the push switch says; read state shares `seenAnnouncements` under `reminder:` ids (`seenState.ts`). Team test-sends: `POST /api/push/test {sessionId}` renders one session to every team device; `POST /api/push/test/reminders {now?, event?, dryRun?}` rehearses the real run for the caller only (same catalogue, window, star lookup, reminders flag, payload and TTL at a mocked clock, sent to the caller's devices, no claim rows). My Devcon (`/ticket`) shows @ethereum.org accounts an "EF internal tools" section (`src/components/internal/EfInternalTools.tsx`: "Ask Deva" opening the DevaBot panel (its header "AI" entry left the public nav on 2026-09-24; state in `src/components/ai/devaBotState.ts`), the Room Screens kiosk link (`/room-screens` left the public nav on 2026-09-24), reset of the install and notification nudges (install-card dismissal plus the per-device push-sheet and Home-card flags), and the "Rehearse reminders" control `ReminderRehearsal`) that drives that route once a minute from the app clock (`useNowMs`, so `?mockNow=` applies) for N minutes.
...
- **Meerkat ids are Pretalx codes**: the Q&A feed and hand-off identify a session by `sourceId` via `meerkatEventId()` (`src/data/meerkat.ts`); the slug is only a one-time retry after a 404 on the code (`SessionQA`), for sessions Meerkat created under it.
