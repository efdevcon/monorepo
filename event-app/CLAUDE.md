...
- **Service worker**: precache stays limited to the app-shell routes, and never `/ticket`: precache is cache-first, and the ticket page must be server-rendered fresh because the root layout's `<link rel="manifest">` is personalised from the session cookie (the install sign-in bridge; a stale copy installs a signed-out app). `/ticket` is warmed into the runtime page cache at SW install instead (`src/sw.ts`). Never enable `skipWaiting`; updates are opt-in via the update toast (`ServiceWorkerUpdater.tsx`).
