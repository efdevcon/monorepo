import type { POI } from '@/types/api-data';

export const poisData: POI[] = [
  {
    name: 'Account Abstraction Hub',
    layerName: 'account-abstraction-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-27-33-208Z-AA-Logo.png',
    description:
      'Connect, learn, and experiment with the latest in account abstraction.',
    websiteLink:
      'https://devconnect.org/community-hubs/account-abstraction-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Ai Onboarding',
    layerName: 'ai-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '1',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'AID Room',
    layerName: 'aid-room',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-50-31-926Z-AID_Room.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '1',
    groupId: '15',
  },
  {
    name: 'Ambulance',
    layerName: 'ambulance',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-50-36-532Z-Ambulance.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '8',
    groupId: '15',
  },
  {
    name: 'Amphitheater',
    layerName: 'amphitheater',
    logo: '',
    description: '',
    websiteLink: '/stages/amphitheater',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '1',
    groupId: '14',
  },
  {
    name: 'Badge Station',
    layerName: 'badge-station',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: null,
    groupId: '11',
  },
  {
    name: 'Base (L2)',
    layerName: 'base',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-20T12-48-11-494Z-base-square.png',
    description:
      'Base is a strategic L2 built by Coinbase that lowers barriers to entry for Ethereum-based dApps by combining scalability, low fees, EVM compatibility and access to a large user base — making it well-suited for the next wave of Web3 adoption.',
    websiteLink: 'https://www.base.org/',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '13',
    groupId: '15',
  },
  {
    name: 'Beer House',
    layerName: 'beer-house',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'BitBase',
    layerName: 'bitbase',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T03-54-09-973Z-BitBase-POI.png',
    description:
      'The simplest way to buy or sell Ethereum and other cryptocurrencies with the help of a specialist.',
    websiteLink: 'https://www.bitbase.es/es',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '12',
  },
  {
    name: 'Bleachers',
    layerName: 'bleachers',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-36-784Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Bleachers',
    layerName: 'poi-bleachers-1-pista-central',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-36-784Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Bleachers',
    layerName: 'poi-bleachers-3-pista-central',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-36-784Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Bleachers',
    layerName: 'poi-bleachers-4-pista-central',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-36-784Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Bleachers',
    layerName: 'poi-bleachers-2-pista-central',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-36-784Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Blue Discussion Corner',
    layerName: 'blue-discussion-corner',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-41-13-056Z-discuss-blue.png',
    description:
      'The Devconnect Discussion Corners are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Discussion-Corners-Booking-299638cdc415803db11bf6c9aa0334f5',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '14',
    groupId: '4',
  },
  {
    name: 'Blue Pavilion Space',
    layerName: 'blue-pavilion',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-05-36-121Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '2',
    groupId: '15',
  },
  {
    name: 'Bootcamp',
    layerName: 'poi-buidIguidl-bootcamp',
    logo: '',
    description: '',
    websiteLink: '/stages/bootcamp',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '14',
  },
  {
    name: 'Ceibo Hall',
    layerName: 'ceibo-hall',
    logo: '',
    description: '',
    websiteLink: '/stages/ceibo',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '12',
    groupId: '14',
  },
  {
    name: 'Chicken Tenders',
    layerName: 'chicken-tenders',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '11',
    groupId: '7',
  },
  {
    name: 'Childcare',
    layerName: 'childcare',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-20-737Z-childcare.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '11',
    groupId: '15',
  },
  {
    name: 'Chillout Area',
    layerName: 'chillout-area',
    logo: '',
    description: 'Sorry – will be closed due to the rain today ☔',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '5',
  },
  {
    name: 'Collectibles Onboarding',
    layerName: 'collectibles-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '2',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'Cowork - Main Arena 1',
    layerName: 'cowork-main-arena-1',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-30-971Z-Cowork.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '2',
  },
  {
    name: 'Cowork - Main Arena 2',
    layerName: 'cowork-main-arena-2',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-35-968Z-Cowork.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '2',
  },
  {
    name: 'Cowork - The Hub',
    layerName: 'cowork-the-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-38-529Z-Cowork.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '14',
    groupId: '2',
  },
  {
    name: 'DeFi Onboarding',
    layerName: 'defi-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '3',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'DeSci Hub',
    layerName: 'desci-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-56-12-379Z-DeSci-Hub.png',
    description: 'Connect with scientists and builders reimagining research.',
    websiteLink: 'https://devconnect.org/community-hubs/desci-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '3',
  },
  {
    name: 'Enterprise Hub',
    layerName: 'enterprise-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-48-59-757Z-Enterprise-Hub.png',
    description: 'Meet teams bringing Ethereum into business.',
    websiteLink: 'https://devconnect.org/community-hubs/enterprise-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Futbol Field',
    layerName: 'futbol-field',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '5',
  },
  {
    name: 'Future Cities & Hubs',
    layerName: 'future-cities-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-54-55-465Z-Future-Cities-Hubs.png',
    description:
      'Explore how cities of the future are being built across urban design and digital systems.',
    websiteLink: 'https://devconnect.org/community-hubs/future-cities-hubs',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Gamers Discussion Corner',
    layerName: 'gamers-discussion-corner',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-41-46-953Z-discuss-gamers.png',
    description:
      'Love all things gaming? Then this Discussion Corner is reserved exclusively for you – swing by and say hey!',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '13',
    groupId: '15',
  },
  {
    name: 'Gaming Onboarding',
    layerName: 'gaming-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '4',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'Governance Geeks Hub',
    layerName: 'governance-geeks-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-05-838Z-Governance-Geeks.png',
    description:
      'Join DAO builders, operators, and researchers to dive into decentralized governance.',
    websiteLink: 'https://devconnect.org/community-hubs/governance-geeks-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '3',
  },
  {
    name: 'Guapaletas',
    layerName: 'guapaletas',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '14',
    groupId: '7',
  },
  {
    name: 'Hamburgueseria 1',
    layerName: 'hamburgueseria-1',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Hamburgueseria 2',
    layerName: 'hamburgueseria-2',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Hardware & Wallets Onboarding',
    layerName: 'hardware-wallets-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '5',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'Infinite Garden Artist Exhibition',
    layerName: 'infinite-garden-artists-exhibition',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-59-40-925Z-infinite-garden.png',
    description:
      'Enjoy a group show titled “Echoing as One,” a collection of artworks that embody and resonate with the vision of the Infinite Garden.',
    websiteLink: 'https://infinitegarden.art/',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '1',
  },
  {
    name: 'Interview Room',
    layerName: 'poi-interview-room',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-41-317Z-Interview_Room.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '8',
  },
  {
    name: 'KOI',
    layerName: 'koi',
    logo: '',
    description: 'Tasty asian food and delicious cocktails 🐉 ❤',
    websiteLink:
      'https://drive.google.com/file/d/1xXQnwhfklTvI4pGQwmOpk7cWJzLAoqo-/view',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '11',
    groupId: '7',
  },
  {
    name: 'L Stage',
    layerName: 'l-stage',
    logo: '',
    description: '',
    websiteLink: '/stages/l',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '12',
    groupId: '14',
  },
  {
    name: 'L2 Onboarding',
    layerName: 'l2-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '6',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'LATAM Hub',
    layerName: 'latam-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-13-292Z-LatamCommunityHub.png',
    description:
      'Join LatAm’s leading Ethereum communities, builders, and educators to co-create learning paths, partnerships, and standards driving real-world adoption.',
    websiteLink: 'https://devconnect.org/community-hubs/latam-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Le Ble',
    layerName: 'le-ble',
    logo: '',
    description:
      "Mobile food truck catering to different situations (events, outdoor events, festivals, etc). It's a synthesis of the best of their bakery and pastry shop, paired with great coffee, always maintaining the quality and warmth that define their service.",
    websiteLink: 'https://www.leble.com.ar/foodtruck.html',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '13',
    groupId: '7',
  },
  {
    name: 'Legal Tech Hub',
    layerName: 'poi-legal-tech-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-25-025Z-Legal-Tech-Hub.png',
    description: 'Explore how law meets innovation.',
    websiteLink: 'https://devconnect.org/community-hubs/legal-tech-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '5',
    groupId: '3',
  },
  {
    name: 'Lightning Talks Stage',
    layerName: 'lighting-talks-stage',
    logo: '',
    description: '',
    websiteLink: '/stages/lightning',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '14',
  },
  {
    name: 'Los Petersen',
    layerName: 'los-petersen',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '8',
    groupId: '7',
  },
  {
    name: 'M1 Stage',
    layerName: 'm1-stage',
    logo: '',
    description: '',
    websiteLink: '/stages/m1',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '14',
  },
  {
    name: 'M2 Stage',
    layerName: 'm2-stage',
    logo: '',
    description: '',
    websiteLink: '/stages/m2',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '14',
  },
  {
    name: 'Media Room',
    layerName: 'media',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-52-123Z-Media_Room.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '8',
  },
  {
    name: 'Meeting Point 1',
    layerName: 'meeting-point-1',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-51-56-038Z-Meeting_point.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '2',
    groupId: '1',
  },
  {
    name: 'Meeting Point 2',
    layerName: 'meeting-point-2',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-00-150Z-Meeting_point.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '1',
  },
  {
    name: 'Meeting Point 3',
    layerName: 'meeting-point-3',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-59-46-784Z-Meeting_point.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '1',
  },
  {
    name: 'Meeting Point 4',
    layerName: 'meeting-point-4',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-59-52-506Z-Meeting_point.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '12',
    groupId: '1',
  },
  {
    name: 'Meeting Point 5',
    layerName: 'meeting-point-5',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-05-42-432Z-Meeting_point.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '1',
  },
  {
    name: 'Meeting Point 6',
    layerName: 'meeting-point-6',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-05-48-696Z-Meeting_point.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '1',
  },
  {
    name: 'Meeting Room 1',
    layerName: 'mr-1',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 10',
    layerName: 'mr-10',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 2',
    layerName: 'mr-2',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 3',
    layerName: 'mr-3',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 4',
    layerName: 'mr-4',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 5',
    layerName: 'mr-5',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 6',
    layerName: 'mr-6',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 7',
    layerName: 'mr-7',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 8',
    layerName: 'mr-8',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room 9',
    layerName: 'mr-9',
    logo: '',
    description:
      'The Devconnect Meeting Rooms are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Devconnect-Meeting-Rooms-Scheduler-292638cdc41581dfa2b0c2ab50b942d0',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Meeting Room A',
    layerName: 'meeting-room-a',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '9',
  },
  {
    name: 'Meeting Room B',
    layerName: 'meeting-room-b',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '9',
  },
  {
    name: 'Meeting Room Registration',
    layerName: 'meeting-rooms-registration',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-05-106Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '9',
  },
  {
    name: 'Music Stage',
    layerName: 'music-stage',
    logo: '',
    description: 'Sorry – will be closed due to the rain today ☔',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '8',
    groupId: '5',
  },
  {
    name: 'Node Operators Hub',
    layerName: 'node-operators-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-32-691Z-Node-Operator.png',
    description:
      'Meet home stakers, client devs, and infra builders exploring Ethereum’s node layer.',
    websiteLink: 'https://devconnect.org/community-hubs/node-operators-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Nogal Hall',
    layerName: 'nogal-hall',
    logo: '',
    description: '',
    websiteLink: '/stages/nogal',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '14',
  },
  {
    name: 'Offices',
    layerName: 'offices',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '15',
  },
  {
    name: 'Onboarding',
    layerName: 'onboarding',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T16-30-33-732Z-Devconnect_ARG_logo_POI.png',
    description:
      'Visit our Devconnect Onboarding Area for information and support, including set up of your Devconnect app.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '4',
    groupId: '10',
  },
  {
    name: 'Onchain Art Hub',
    layerName: 'onchain-art-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-38-042Z-Onchain-Art-Hub.png',
    description:
      'Connect with onchain artists and collectives from across Argentina.',
    websiteLink: 'https://devconnect.org/community-hubs/onchain-art-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Onchain Fintech Hub',
    layerName: 'onchain-fintech-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-54-49-797Z-Onchain-Fintech.png',
    description:
      'Join fintech and crypto builders creating real onchain banking experiences.',
    websiteLink: 'https://devconnect.org/community-hubs/onchain-fintech-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Open Air Cinema',
    layerName: 'open-air-cinema',
    logo: '',
    description: 'Sorry – will be closed due to the rain today ☔',
    websiteLink: '/stages/outdoor-cinema',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '5',
  },
  {
    name: 'Open Source Hub',
    layerName: 'open-source-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-29T12-19-06-234Z-OS_Hub_2.png',
    description:
      'Collaborate with developers keeping Ethereum open, resilient, and community-driven.',
    websiteLink: 'https://devconnect.org/community-hubs/open-source-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Orange Discussion Corner',
    layerName: 'orange-discussion-corner',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-41-49-799Z-discuss-orange.png',
    description:
      'The Devconnect Discussion Corners are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Discussion-Corners-Booking-299638cdc415803db11bf6c9aa0334f5',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '14',
    groupId: '4',
  },
  {
    name: 'Outdoor Seating Area',
    layerName: 'outdoor-seating-area-1',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-39-200Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Outdoor Seating Area',
    layerName: 'outdoor-seating-area',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T23-10-39-200Z-seating-area-bleachers.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '15',
  },
  {
    name: 'Persicco',
    layerName: 'persicco',
    logo: '',
    description:
      'Persicco was founded in 2001, but its roots trace back to the legendary Scannapieco ice cream parlor, which opened its doors in 1938 in the Colegiales neighborhood of Buenos Aires. A must try for ice cream lovers.',
    websiteLink: 'https://persicco.com.ar/sabores.html',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '7',
  },
  {
    name: 'Power Up - Entertainment 1',
    layerName: 'power-up-station-entertainment-1',
    logo: '',
    description: 'Sorry – will be closed due to the rain today ☔',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '7',
  },
  {
    name: 'Power Up - Entertainment 2',
    layerName: 'power-up-station-entertainment-2',
    logo: '',
    description: 'Sorry – will be closed due to the rain today ☔',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '7',
  },
  {
    name: 'Power Up - Green Pavilion 1',
    layerName: 'power-up-station-green-pavilion-1',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Power Up - Green Pavilion 2',
    layerName: 'power-up-station-green-pavilion-2',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Power Up - Green Pavilion 3',
    layerName: 'power-up-station-green-pavilion-3',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Power Up - Green Pavilion 4',
    layerName: 'power-up-station-green-pavilion-4',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Power Up - Green Pavilion 5',
    layerName: 'power-up-station-green-pavilion-5',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Power Up - Green Pavilion 6',
    layerName: 'power-up-station-green-pavilion-6',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '7',
  },
  {
    name: 'Power Up - Main Arena 1',
    layerName: 'power-up-station-main-arena-1',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '7',
  },
  {
    name: 'Power Up - Main Arena 2',
    layerName: 'power-up-station-main-arena-2',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '7',
  },
  {
    name: 'Power Up - Main Arena 3',
    layerName: 'power-up-station-main-arena-3',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '7',
  },
  {
    name: 'Power Up - Main Arena 4',
    layerName: 'power-up-station-main-arena-4',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '7',
  },
  {
    name: 'Power Up - Red Pavilion',
    layerName: 'power-up-station-red-pavilion',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '12',
    groupId: '7',
  },
  {
    name: 'Power Up - The Cave',
    layerName: 'power-up-station-the-cave',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '13',
    groupId: '7',
  },
  {
    name: 'Power Up - Yellow Pavilion',
    layerName: 'power-up-station-yellow-pavilion',
    logo: '',
    description:
      'Power-up and recharge for the Ethereum World’s Fair at our dedicated coffee & snack stations.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '7',
  },
  {
    name: 'Power Up Station 13',
    layerName: 'power-up-station-13',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '11',
  },
  {
    name: 'Privacy Hub',
    layerName: 'privacy-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-49-711Z-privacy-hub.png',
    description: 'Explore how privacy research meets real-world use.',
    websiteLink: 'https://devconnect.org/community-hubs/privacy-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '13',
    groupId: '3',
  },
  {
    name: 'Privacy Onboarding',
    layerName: 'privacy-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '7',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'Purple Discussion Corner',
    layerName: 'purple-discussion-corner',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-17T00-13-38-749Z-discuss-purple.png',
    description:
      'The Devconnect Discussion Corners are open to all attendees who wants to work, network, and meet the people working in Ethereum. ',
    websiteLink:
      'https://ef-events.notion.site/Discussion-Corners-Booking-299638cdc415803db11bf6c9aa0334f5',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '4',
  },
  {
    name: 'QR code 1',
    layerName: 'qr-code-1',
    logo: '',
    description: 'map qr code',
    websiteLink: '/map',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: null,
    groupId: '13',
  },
  {
    name: 'QR code 2',
    layerName: 'qr-code-2',
    logo: '',
    description: '',
    websiteLink: '/map',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: null,
    groupId: '13',
  },
  {
    name: 'Quiet Cowork',
    layerName: 'quiet-cowork',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-19-713Z-Cowork.png',
    description:
      'The place for focus work. We respectfully ask that you turn your phone to silent in this space.',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '11',
    groupId: '2',
  },
  {
    name: 'Real World Assets Hub',
    layerName: 'poi-real-world-assets-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-57-230Z-RWA-Hub.png',
    description:
      'Explore how assets from the physical world are moving onchain.',
    websiteLink: 'https://devconnect.org/community-hubs/real-world-assets-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'Regen Hub',
    layerName: 'regen-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-55-54-012Z-Regen-Hub.png',
    description:
      'Discover projects using Ethereum to restore ecosystems and drive climate action.',
    websiteLink: 'https://devconnect.org/community-hubs/regen-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '7',
    groupId: '3',
  },
  {
    name: 'Registration',
    layerName: 'registration',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-22-826Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '4',
    groupId: '1',
  },
  {
    name: 'Registration - M1 1',
    layerName: 'poi-registration-yellow-pavilion-m1-1',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-31-446Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '15',
  },
  {
    name: 'Registration - M1 2',
    layerName: 'poi-registration-yellow-pavilion-m1-2',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-38-045Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '15',
  },
  {
    name: 'Registration - M2 Stage',
    layerName: 'poi-registration-yellow-pavilion-m2',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-41-286Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '15',
  },
  {
    name: 'Registration - XL Stage',
    layerName: 'poi-registration-yellow-pavilion-xl',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-44-730Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '15',
  },
  {
    name: 'Registration - XS Stage',
    layerName: 'poi-registration-yellow-pavilion-xs',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-47-705Z-Registration.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '15',
  },
  {
    name: 'Ripio - Green Pavilion',
    layerName: 'ripio-green-pavilion',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T03-54-17-351Z-Ripio-POI.png',
    description:
      'Onramp with the leading crypto company in LATAM. They’re building tools that bring DeFi to millions across the region, making automated yield accessible to everyone.',
    websiteLink: 'https://www.ripio.com/en',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '12',
  },
  {
    name: 'Ripio - The Hub',
    layerName: 'ripio-the-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T03-54-20-279Z-Ripio-POI.png',
    description:
      'Onramp with the leading crypto company in LATAM. They’re building tools that bring DeFi to millions across the region, making automated yield accessible to everyone.',
    websiteLink: 'https://www.ripio.com/en',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '14',
    groupId: '12',
  },
  {
    name: 'Social Onboarding',
    layerName: 'social-onboarding',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: '8',
    locationId: null,
    groupId: '11',
  },
  {
    name: 'Storage',
    layerName: 'storage',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '3',
    groupId: '15',
  },
  {
    name: 'SWAG station',
    layerName: 'swag-station',
    logo: '',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '6',
  },
  {
    name: 'Toilet - Blue Pavilion',
    layerName: 'toilet-blue-pavilion',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-52-54-204Z-toilets.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '2',
    groupId: '16',
  },
  {
    name: 'Toilet - Green Pavilion',
    layerName: 'toilet-green-pavilion',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-53-01-112Z-toilets.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '16',
  },
  {
    name: 'Toilet - Quiet Cowork 1',
    layerName: 'toilet-quiet-cowork-1',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-53-04-504Z-toilets.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '11',
    groupId: '16',
  },
  {
    name: 'Toilet - Quiet Cowork 2',
    layerName: 'toilet-quiet-cowork-2',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-53-07-567Z-toilets.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '11',
    groupId: '16',
  },
  {
    name: 'Toilet - Red Pavilion',
    layerName: 'toilet-red-pavilion',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-53-11-545Z-toilets.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '12',
    groupId: '16',
  },
  {
    name: 'Toilet - Yellow Pavilion',
    layerName: 'toilet-yellow-pavilion',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-11-16T22-53-15-036Z-toilets.png',
    description: '',
    websiteLink: '',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '16',
  },
  {
    name: 'Women in Web3 Hub',
    layerName: 'women-in-web3-hub',
    logo: 'https://mealmslwugsqqyoesrxd.supabase.co/storage/v1/object/public/notion/2025-10-20T17-56-01-631Z-Women-In-Web-3.png',
    description: 'Connect, learn, and grow with women and allies in Web3.',
    websiteLink: 'https://devconnect.org/community-hubs/women-in-web3-hub',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '6',
    groupId: '3',
  },
  {
    name: 'XL Stage',
    layerName: 'xl-stage',
    logo: '',
    description: '',
    websiteLink: '/stages/xl',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '14',
  },
  {
    name: 'XS Stage',
    layerName: 'xs-stage',
    logo: '',
    description: '',
    websiteLink: '/stages/xs',
    twitterLink: '',
    farcasterLink: '',
    districtId: null,
    locationId: '15',
    groupId: '14',
  },
];
