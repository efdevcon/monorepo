...
import poapDropStats from '@/data/poap-drop-stats.json';
...
// POAP mint statistics for drops from quests. These were queried live from the
// POAP Compass API (public.compass.poap.tech), which died with the POAP project
// shutdown. Minting ended with the event, so the final counts now come from the
// snapshot in src/data/poap-drop-stats.json (generated by
// scripts/build-poap-static-data.ts) and the static quests data.
async function fetchPOAPMintStats(): Promise<POAPMintStats | null> {
  try {
    const dropStats = poapDropStats as Record<string, { name: string; mintCount: number }>;

    let totalMints = 0;
    const drops: POAPDrop[] = [];

    questsData
      .filter(quest => quest.conditionType === 'verifyPoap' && quest.conditionValues)
      .forEach(quest => {
        const dropId = parseInt(quest.conditionValues);
        if (isNaN(dropId)) return;

        const stats = dropStats[String(dropId)];
        const poapCount = stats?.mintCount || 0;
        totalMints += poapCount;

        const supporterId = quest.supporterId || undefined;
        const supporter = supporterId ? supportersData[supporterId] : null;
        const districtId = supporter?.districtId || undefined;

        // Include 0 count drops to show all quest POAPs
        drops.push({
          id: dropId,
          name: stats?.name || quest.name,
          mintCount: poapCount,
    ...
    console.error('Error computing POAP mint stats:', error);
    return null;
  }
}
