    ...
    async (targetAddress: string, force: boolean = false) => {
      ...
      try {
        ...
        const resolvedIdentity: WalletIdentity = {
          name,
          avatar,
          worldfairName: worldfairNftName, // Always include worldfair.eth NFT ownership (may be null)
        };
        ...
        console.log(
          `✅ [WALLET_MANAGER] [identity] Identity resolved for ${addressKey}:`,
          {
            ...
            worldfairName: worldfairNftName, // Separately tracked worldfair.eth NFT
          }
        );
      } catch (err) {
